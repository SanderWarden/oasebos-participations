...
final class TemplatePreviewController
{
    ...
    {
        add_action('wp_ajax_oasebos_template_preview', [$this, 'ajaxPreview']);
        add_action('admin_post_oasebos_template_pdf_preview', [$this, 'downloadPdfPreview']);
        add_action('admin_post_oasebos_certificate_pdf_export', [$this, 'exportCertificatePdf']);
    }

    public function ajaxPreview(): void
    {
        if (! current_user_can(Capabilities::MANAGE) && ! current_user_can('manage_options')) {
            wp_send_json_error(['message' => __('Je hebt geen toestemming om templates te bekijken.', 'oasebos-participations')], 403);
        }
        if (! check_ajax_referer(Nonces::ADMIN_ACTION, 'nonce', false)) {
            wp_send_json_error(['message' => __('Beveiligingscontrole mislukt.', 'oasebos-participations')], 403);
        }
    ...
    public function downloadPdfPreview(): void
    {
        Capabilities::requireManage();
        Nonces::verifyAdmin();

        if (! class_exists('\\Dompdf\\Dompdf')) {
            wp_die(esc_html__('Dompdf is niet beschikbaar voor PDF-preview.', 'oasebos-participations'));
        }
        ...
        header('Content-Disposition: inline; filename="oasebos-template-preview.pdf"');
        echo $dompdf->output();
        exit;
    }

    public function exportCertificatePdf(): void
    {
        Capabilities::requireManage();
        Nonces::verifyAdmin();

        if (! class_exists('\Dompdf\Dompdf')) {
            wp_die(esc_html__('Dompdf is niet beschikbaar voor PDF-export.', 'oasebos-participations'));
        }
        ...
        nocache_headers();
        header('Content-Type: application/pdf');
        header('Content-Disposition: attachment; filename="oasebos-certificaat-preview.pdf"');
        echo $dompdf->output();
        exit;
    }

    private function previewPayload(array $source): array
    {
        $renderer = new TemplateRenderer();
        $type = Sanitizer::text('type', $source, 'agreement');
        $content = Sanitizer::html('content', $source, '');
        $css = Sanitizer::textarea('css', $source, '');
        $rendered = $renderer->render($content, $renderer->sampleContext(), 'pdf');
        if ('certificate' === $type) {
    ...
    private function previewTitle(string $type): string
    {
        return match ($type) {
            'certificate' => __('Certificaat preview', 'oasebos-participations'),
            'email' => __('E-mail preview', 'oasebos-participations'),
            default => __('Overeenkomst preview', 'oasebos-participations'),
        };
    }
...
';
    ...
    private function normalizeCertificateImages(string $html): string
    {
        ...
        $html = $this->normalizeImageBox($html, 'logo-placeholder', '42mm', '22mm');
    ...
    private function normalizeImageBox(string $html, string $class, string $width, string $height): string
    {
        ...
        return preg_replace_callback($pattern, static function (array $matches) use ($class, $width, $height): string {
            ...
            return '<div class="' . $class . ' has-image"><img' . $attrs . ' style="display:block;width:' . $width . ';height:auto;max-width:' . $width . ';max-height:' . $height . ';margin:0 auto;border:0;"></div>';
        }, $html) ?? $html;
    }
...
';
    ...
    private function certificatePreviewCss(): string
    {
        return '
.oasebos-pdf-preview-page .logo-placeholder.has-image { padding: 0 !important; background: transparent !important; border: 0 !important; line-height: 0 !important; font-size: 0 !important; }
