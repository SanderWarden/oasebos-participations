...
<div class="oasebos-participation-landing" data-oasebos-landing data-form-url="<?php echo esc_url($formPageUrl ?: $defaultFormUrl); ?>">
    <div class="oasebos-landing-layout">
        <aside class="oasebos-basket" data-oasebos-basket>
            <div class="oasebos-basket__header"><strong><?php esc_html_e('Het mandje', 'oasebos-participations'); ?></strong><span data-oasebos-basket-count><?php echo esc_html((string) array_sum($basketItems)); ?></span></div>
            <ul class="oasebos-basket__items" data-oasebos-basket-items>
                <?php if ($basketItems): ?>
                    <?php foreach ($basketItems as $projectId => $units): $itemProject = $projectsById[$projectId]; ?>
                        ...
                        <li data-basket-project-id="<?php echo esc_attr((string) $projectId); ?>"><span><?php echo esc_html((string) $itemProject['name']); ?><small><?php echo esc_html(number_format_i18n($totalHectares, 4)); ?> ha</small></span><strong><?php echo esc_html($itemCost); ?></strong></li>
                    <?php endforeach; ?>
                <?php else: ?>
        ...
        <?php foreach ($projects as $project): ?>
            ...
            <article class="oasebos-project-card <?php echo isset($basketItems[(int) $project['id']]) ? 'is-selected' : ''; ?>" data-project-id="<?php echo esc_attr((string) $project['id']); ?>" data-project-name="<?php echo esc_attr((string) $project['name']); ?>" data-project-unit-size="<?php echo esc_attr((string) $project['unit_size']); ?>" data-project-price="<?php echo esc_attr((string) $project['price_per_unit']); ?>" data-project-currency="<?php echo esc_attr((string) $project['currency']); ?>" data-project-units="<?php echo esc_attr((string) ($basketItems[(int) $project['id']] ?? 0)); ?>">
                <div class="oasebos-project-card__body">
                    <h3><?php echo esc_html((string) $project['name']); ?></h3>
                    <?php if (! empty($project['location'])): ?><p class="oasebos-project-location"><?php echo esc_html((string) $project['location']); ?></p><?php endif; ?>
                    <?php if (! empty($project['description'])): ?><p><?php echo esc_html(wp_trim_words((string) $project['description'], 28)); ?></p><?php endif; ?>
                    <dl class="oasebos-project-meta">
                        <div><dt><?php esc_html_e('Beschikbaar', 'oasebos-participations'); ?></dt><dd><?php echo esc_html(number_format_i18n((float) $project['available_hectares'], 2)); ?> ha</dd></div>
                        <div><dt><?php esc_html_e('Eenheid', 'oasebos-participations'); ?></dt><dd><?php echo esc_html(number_format_i18n((float) $project['unit_size'], 4)); ?> ha</dd></div>
                        <div><dt><?php esc_html_e('Prijs', 'oasebos-participations'); ?></dt><dd><?php echo esc_html($price); ?></dd></div>
                    </dl>
                </div>
                <?php if ($availableUnits >= 1): ?>
                    <a class="oasebos-add-project" href="<?php echo esc_url($addUrl); ?>" data-oasebos-add-project aria-label="<?php echo esc_attr(sprintf(__('Voeg een eenheid van %s toe', 'oasebos-participations'), (string) $project['name'])); ?>">+</a>
                <?php else: ?>
                    <button type="button" class="oasebos-add-project" disabled aria-label="<?php echo esc_attr(sprintf(__('Geen eenheden beschikbaar voor %s', 'oasebos-participations'), (string) $project['name'])); ?>">+</button>
                <?php endif; ?>
            </article>
        <?php endforeach; ?>
        </div>
